...
def delete_image
  ...
  @images = @property.images.paginate(:page => params[:page],:per_page => 8,:conditions => ["parent_id IS NULL"])
  ...
  def view_property
    ...
    if @property.images.size > 0
      @images = @property.images.paginate(:page => params[:page],:per_page => 3,:conditions => ["parent_id IS NULL"])
      @image = Image.find_by_id(@images.first.id)
      puts "-----------------> #{@images}"
    end
